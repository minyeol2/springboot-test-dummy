...
public class LogService {
    ...
    public void registerLogSync_2(Long referencdId) {
        Thread mainThread = Thread.currentThread();
        log.info("[ThreadID={}]",mainThread.getId() + " : Log서비스 로그저장 메서드 실행");
        Orders orders = orderRepository.findById(referencdId).orElseThrow();

        String logMessage = String.format("주문상품 : [%d], 주문수량 : [%d],  비고내용 : [%s]", orders.getOrderItemId(), orders.getOrderQuantity(), orders.getRemark());
        Log logs = LogDTO.builder().referenceId(orders.getId()).tableName("ORDERS").logMessage(logMessage).build().toEntity();
        logRepository.save(logs);
        //throwNPE("LogService에서 강제발생");
        log.info("[ThreadID={}]",mainThread.getId() + " : Log서비스 로그저장 메서드 종료");
    }
